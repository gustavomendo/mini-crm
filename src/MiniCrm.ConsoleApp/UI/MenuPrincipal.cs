using MiniCrm.ConsoleApp.Data;
using MiniCrm.ConsoleApp.Modelos;
using MiniCrm.ConsoleApp.Services;
namespace MiniCrm.ConsoleApp.Ui;

internal class MenuPrincipal
{
    private readonly ClienteRepositorio repositorio;
    private readonly ClienteServico servico;

    public MenuPrincipal(ClienteRepositorio repositorio, ClienteServico servico)
    {
        this.repositorio = repositorio;
        this.servico = servico;
    }

    public void Executar()
    {
        string opcao;
        do
        {
            Console.Clear();
            Console.WriteLine("=== MINI CRM ===");
            Console.WriteLine("1. Listar Clientes");
            Console.WriteLine("2. Adicionar Cliente");
            Console.WriteLine("3. Procurar Cliente por Nome");
            Console.WriteLine("4. Remover Cliente");
            Console.WriteLine("5. Procurar Cliente por Id");
            Console.WriteLine("0. Sair");
            Console.Write("\nEscolhe uma opção: ");

            opcao = Console.ReadLine() ?? "0";

            switch (opcao)
            {
                case "1":
                    Console.WriteLine("\n[A carregar a lista de clientes...]\n");
                    Thread.Sleep(500);
                    MostrarClientes();
                    break;

                case "2":
                    Console.WriteLine("\n[A adicionar um novo cliente...]\n");
                    CriarCliente();
                    break;

                case "3":
                    Console.WriteLine("\n[A procurar o cliente...]\n");
                    ProcurarClientePorNome();
                    Thread.Sleep(500);
                    break;

                case "4":
                    Console.WriteLine("\n[A remover o cliente...]\n");
                    RemoverCliente();
                    Thread.Sleep(500);
                    break;
                case "5":
                    ProcurarClientePorId();
                    break;

                case "0":
                    Console.WriteLine("\nA sair do programa. Até logo!");
                    break;

                default:
                    Console.WriteLine("\nOpção inválida! Tenta novamente.");
                    break;
            }

            if (opcao != "0")
            {
                Console.WriteLine("\nPressiona qualquer tecla para continuar...");
                Console.ReadKey();
            }
        } while (opcao != "0");
    }
    private void MostrarClientes()
    {
        List<Cliente> clientes = repositorio.ListarClientes();

        if (clientes.Count == 0)
        {
            Console.WriteLine("Não existem clientes cadastrados.");
            return;
        }

        string cabecalho = $"{"#",-3} | {"Id",-8} | {"Nome",-20} | {"Email",-27} | {"Data Registo",-12}";
        Console.WriteLine(cabecalho);
        Console.WriteLine(new string('-', cabecalho.Length));

        int contador = 1;
        foreach (Cliente cliente in clientes)
        {
            Console.WriteLine($"{contador,-3} | {cliente.Id.ToString().Substring(0, 8),-8} | {cliente.Nome,-20} | {cliente.Email,-27} | {cliente.DataRegisto,-12}");
            contador++;
        }

        string mensagemTotal = clientes.Count == 1 ? "cliente cadastrado" : "clientes cadastrados";
        Console.WriteLine($"Total: {clientes.Count} {mensagemTotal}.");
    }
    private void ProcurarClientePorNome()
    {
        Console.Write("Indique o nome do Cliente: ");
        string nome = Console.ReadLine() ?? "";

        List<Cliente> encontrados = repositorio.ProcurarPorNome(nome);

        if (encontrados.Count == 0)
        {
            Console.WriteLine("Cliente não encontrado");
            return;
        }

        foreach (Cliente cliente in encontrados)
        {
            Console.WriteLine(cliente);
        }

        Console.WriteLine($"Foram encontrados: {encontrados.Count}");
    }
    private void ProcurarClientePorId()
    {
        Console.Write("Indique o Id do cliente (basta o início): ");
        string idEscrito = Console.ReadLine() ?? "";

        if (string.IsNullOrWhiteSpace(idEscrito))
        {
            Console.WriteLine("Tem de indicar um Id.");
            return;
        }

        List<Cliente> encontrados = repositorio.ProcurarPorId(idEscrito);

        if (encontrados.Count == 0)
        {
            Console.WriteLine("Não existe nenhum cliente com esse Id.");
            return;
        }

        if (encontrados.Count > 1)
        {
            Console.WriteLine($"Esse início de Id corresponde a {encontrados.Count} clientes. Escreva mais caracteres.");
            return;
        }

        Console.WriteLine(encontrados[0]);
    }
    private void RemoverCliente()
    {
        List<Cliente> clientes = repositorio.ListarClientes();

        if (clientes.Count == 0)
        {
            Console.WriteLine("Não existem clientes para remover.");
            return;
        }

        MostrarClientes();

        Console.Write("Indique o número do cliente a remover: ");
        string numeroEscrito = Console.ReadLine() ?? "";

        if (!int.TryParse(numeroEscrito, out int numeroCliente))
        {
            Console.WriteLine("Tem de indicar um número.");
            return;
        }

        if (numeroCliente < 1 || numeroCliente > clientes.Count)
        {
            Console.WriteLine("Não existe nenhum cliente com esse número.");
            return;
        }

        Cliente cliente = clientes[numeroCliente - 1];

        Console.Write($"Vai remover: {cliente.Nome}. Tem a certeza? (s/n): ");
        string confirmacao = Console.ReadLine() ?? "";

        if (confirmacao.Equals("s", StringComparison.OrdinalIgnoreCase))
        {
            repositorio.RemoverPorId(cliente.Id);
            Console.WriteLine($"Cliente \"{cliente.Nome}\" removido com sucesso.");
        }
        else
        {
            Console.WriteLine("Remoção cancelada.");
        }
    }
    private void CriarCliente()
    {
        string nome;
        while (true)
        {
            Console.Write("Indique o Nome: ");
            nome = Console.ReadLine() ?? "";

            if (string.IsNullOrWhiteSpace(nome))
            {
                Console.WriteLine("O nome não pode ficar vazio.");
                continue;
            }

            break;
        }

        string email;
        while (true)
        {
            Console.Write("Indique o Email: ");
            email = Console.ReadLine() ?? "";

            if (string.IsNullOrWhiteSpace(email))
            {
                Console.WriteLine("O email não pode ficar vazio.");
                continue;
            }

            if (servico.EmailJaExiste(email))
            {
                Console.WriteLine("Já existe um cliente com esse Email.");
                continue;
            }

            break;
        }

        Cliente cliente = servico.Adicionar(nome, email);

        Console.WriteLine($"\nCliente \"{cliente.Nome}\" adicionado com sucesso.");
    }
}